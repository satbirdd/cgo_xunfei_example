...
char* Entry(char* argv)
{
	int			ret						=	MSP_SUCCESS;
	int			upload_on				=	1; //是否上传用户词表
	const char* login_params			=	"appid = 597ae591, work_dir = ."; // 登录参数，appid与msc库绑定,请勿随意改动
	...
	*/
	...
	// 	printf("上传用户词表成功\n");
	// }
	// run_iat("/home/liulei/works/demo/bin/wav/out2.wav", session_begin_params); //iflytek02音频内容为“中美数控”；如果上传了用户词表，识别结果为：“中美速控”。
	return run_iat(argv, session_begin_params); //iflytek02音频内容为“中美数控”；如果上传了用户词表，识别结果为：“中美速控”。
// exit:
// 	// printf("按任意键退出 ...\n");
// 	// getchar();
// 	MSPLogout(); //退出登录
